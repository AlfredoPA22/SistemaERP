...
namespace Logica
{
    ...
    {
        public List<ECompra> listarNota(int idempresa)
        {
            ...
            using (var esquema = GetEsquema())
            {
                try
                {
                    ...
                    if (notas != null)
                    {
                        ...
                        c.IdProveedor = notas.IdProveedor;
                        c.IdCliente = notas.IdCliente;
                        c.IdVendedor = notas.IdVendedor;
                    }
                    else
                    {
                ...
                try
                {
                    ...
                    foreach (var i in lnota)
                    {
                        ...
                        foreach(var a in larticulo)
                        {
                            ...
                            e.Articulo = a.Nombre;
                            e.CodigoArticulo = a.CodigoArticulo;
                ...
                try
                {
                    ...
                    foreach (var i in lnota)
                    {
                        ...
                        foreach (var a in larticulo)
                        {
                            ...
                            if (i.IdSerie != null)
                            {
                                var detalleserie = (from x in esquema.ArticuloSerie
                                                    where x.IdArticuloSerie == i.IdSerie
                                                    select x).FirstOrDefault();
                                e.NroSerie = detalleserie.NumeroSerie;
                            }
                            ...
                            if (periodo != null)
                            {
                                ...
                                if (Entidad.Tipo == (int)TipoNota.Compra)
                                {
                                    ...
                                    if (notas.Count() == 0)
                                    {
                                        ...
                                        foreach (var ia in eDetalleComprobante2)
                                        {
                                            ArticuloSerie p = new ArticuloSerie();
                                            p.IdArticulo = ia.IdArticulo;
                                            p.IdNota = Entidad.IdNota;
                                            p.NroLote = 1;
                                            p.NumeroSerie = ia.NroSerie;
                                            p.Estado = 1;
                                            esquema.ArticuloSerie.Add(p);
                                            esquema.SaveChanges();
                                        }
                                    }
                                    else if (notas.Count() > 0)
                                    {
                                        ...
                                        foreach (var ia in eDetalleComprobante2)
                                        {
                                            var lote = (from lot in esquema.Lote
                                                        where lot.IdNota == Entidad.IdNota && lot.IdArticulo == ia.IdArticulo
                                                        select lot).FirstOrDefault();
                                            ArticuloSerie p = new ArticuloSerie();
                                            p.IdArticulo = ia.IdArticulo;
                                            p.IdNota = Entidad.IdNota;
                                            p.NroLote = lote.NroLote;
                                            p.NumeroSerie = ia.NroSerie;
                                            p.Estado = 1;
                                            esquema.ArticuloSerie.Add(p);
                                            esquema.SaveChanges();
                    ...
                    else
                    {
                        if (Entidad.Tipo == (int)TipoNota.Compra)
                        {
                            ...
                            else if (notas.Count() > 0)
                            {
                                ...
                                {
                                    var lote = (from x in esquema.Lote
                                                where x.IdNota==Entidad.IdNota && x.IdArticulo==ia.IdArticulo
                                                select x).FirstOrDefault();
                            ...
                            if (periodo != null)
                            {
                                ...
                                if (Entidad.Tipo == (int)TipoNota.Venta)
                                {
                                    ...
                                    foreach (var i in eDetalleComprobante)
                                    {
                                        ...
                                        if (i.IdSerie > 0)
                                        {
                                            var nroserie = (from arserie in esquema.ArticuloSerie
                                                            where arserie.IdArticuloSerie == i.IdSerie
                                                            select arserie).FirstOrDefault();
                                            nroserie.Estado = 3;
                                        }
        ...
        //reportes
        public List<ERDatosBasicoCuenta> ReporteDatosBasicoNota(string usuario,int idempresa)
        {
            using (var esquema = GetEsquema())
            {
                try
                {
                    ...
                    eRDatosBasico.NombreEmpresa = empresa.Nombre;
                    eRDatosBasico.Direccion = empresa.Direccion;
                    eRDatosBasico.Telefono = empresa.Telefono;
                    eRDatosBasico.Correo = empresa.Correo;

                    if (empresa.Imagen != null)
                    {
                        eRDatosBasico.RutaImagen = Encoding.UTF8.GetString(empresa.Imagen);
                    }
        ...
        public List<ERDatosBasicoMargenXMes> ReporteDatosBasicoMargenXMes(string usuario, int idempresa, int idperiodo)
        {
            using (var esquema = GetEsquema())
            {
                try
                {
                    ...
                    List<ERDatosBasicoMargenXMes> basicos = new List<ERDatosBasicoMargenXMes>();
                    ERDatosBasicoMargenXMes eRDatosBasico = new ERDatosBasicoMargenXMes();
                    eRDatosBasico.Usuario = usuario;
                    ...
                    eRDatosBasico.FechaInicioPeriodo = periodo.Fechainicio.ToString("dd/MM/yyyy");
                    eRDatosBasico.FechaFinPeriodo = periodo.Fechafin.ToString("dd/MM/yyyy");
                ...
                try
                {
                    ...
                    if (comprobantes != null)
                    {
                        ...
                        else if(comprobantes.Tipo == (int)TipoNota.Venta)
                        {
                            c.TipoNota = "Venta";
                            var cliente = (from x in esquema.Cliente
                                                where x.IdCliente == comprobantes.IdCliente
                                                select x).FirstOrDefault();
                            var Vendedor = (from x in esquema.Vendedor
                                           where x.IdVendedor == comprobantes.IdVendedor
                                           select x).FirstOrDefault();
                            c.Cliente = cliente.Nombre+cliente.Apellido;
                            c.Vendedor= Vendedor.Nombre+Vendedor.Apelldio;
        ...
        public List<ENota> ObtenerNotasComprasReporte(int idempresa)
        {
            using (var esquema = GetEsquema())
            {
                ...
                try
                {
                    ...
                    if (comprobantes != null)
                    {
                        foreach(var i in comprobantes)
                        {
                            ENota c = new ENota();
                            c.NroNota = i.NroNota;
                            c.Fecha = i.Fecha.ToString("dd/MM/yyyy");
                            var proveedor = (from x in esquema.Proveedor
                                           where x.IdProveedor == i.IdProveedor
                                           select x).FirstOrDefault();
                            c.Proveedor = proveedor.Nombre;
                    ...
                    {
                        throw new MensageException("Error no se pudo obtener las notas de compras");
                    }
        ...
        public List<ENota> ObtenerNotasVentasReporte(int idempresa)
        {
            using (var esquema = GetEsquema())
            {
                ...
                try
                {
                    ...
                    {
                        throw new MensageException("Error no se pudo obtener las notas de Ventas");
                    }
        ...
        public List<ENota> ObtenerMargenXMesReporte(int idempresa, int idgestion,int idperiodo)
        {
            using (var esquema = GetEsquema())
            {
                ...
                try
                {
                    ...
                    var comprobantes = (from x in esquema.Nota
                                        where x.IdEmpresa == idempresa && x.Tipo == (int)TipoNota.Venta && x.Fecha>=Periodo.Fechainicio && x.Fecha<=Periodo.Fechafin && x.Estado==(int)EstadoNota.Activo
                                        select x).ToList();
                    ...
                    if (comprobantes != null)
                    {
                        foreach (var i in comprobantes)
                        {
                            ...
                            {
                                var lote = (from x in esquema.Lote
                                                where x.IdArticulo == d.IdArticulo && x.NroLote==d.NroLote
                                                select x).FirstOrDefault();

                                margen = ((margen) + (d.PrecioVenta - lote.PrecioCompra));
                            }
                            ENota c = new ENota();
                            c.NroNota = i.NroNota;
                            c.Fecha = i.Fecha.ToString("dd/MM/yyyy");
        ...
        public double listarComprasTotal(int idempresa)
        {
            using (var esquema = GetEsquema())
            {
                ...
                {
                    throw new MensageException("Error no se puedo obtener el total de compras");
                }

            }
        }
        public double listarVentasTotal(int idempresa)
        {
            using (var esquema = GetEsquema())
            {
                ...
                try
                {
                    var notasventa = (from x in esquema.Nota
                                       where x.IdEmpresa == idempresa && x.Tipo == (int)TipoNota.Venta && x.Estado == (int)EstadoNota.Activo
                                       select x).ToList();
                    double total = 0;
                    foreach (var i in notasventa)
                    {
                ...
                {
                    throw new MensageException("Error no se puedo obtener el total de ventas");
                }

            }
        }
        public List<ENroSerie> listarSerieXNota(int idnota, int idempresa)
        {
            using (var esquema = GetEsquema())
            {
                ...
                try
                {
                    ...
                    List<ENroSerie> detalleComprobantes = new List<ENroSerie>();


                    var dcomprobantes = (from x in esquema.ArticuloSerie
                                         where x.IdNota == idnota
                                         select x).ToList();
                    ...
                    foreach (var i in dcomprobantes)
                    {
                        ...
                        ENroSerie e = new ENroSerie();
                        e.IdArticulo = i.IdArticulo;
                        e.Articulo = producto.Nombre;
                        e.NroLote = i.NroLote;
                        e.IdNota = i.IdNota;
                        e.NroSerie = i.NumeroSerie;
                        detalleComprobantes.Add(e);
        ...
        public ArticuloSerie listarSerieXId(int idserie)
        {
            using (var esquema = GetEsquema())
            {
